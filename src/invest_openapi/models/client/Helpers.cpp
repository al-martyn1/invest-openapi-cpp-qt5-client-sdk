...
public:
    ...
    static SerializerSettings *getInstance(){
        if(instance == nullptr){
            instance = new SerializerSettings();
        }
        return instance;
    }
private:
    explicit SerializerSettings(){
        instance = this;
        dateTimeFormat.clear();
    }
    static SerializerSettings *instance;
    QString dateTimeFormat;
};

SerializerSettings * SerializerSettings::instance = nullptr;
...
bool setDateTimeFormat(const QString& dateTimeFormat){
    ...
    if(dt.isValid()){
        success = true;
        SerializerSettings::setDateTimeFormat(dateTimeFormat);
    }
    return success;
}
...
bool fromStringValue(const QString &inStr, QDateTime &value) {
    if (inStr.isEmpty()) {
        ...
        auto dateTime = SerializerSettings::getInstance()->getDateTimeFormat().isEmpty()?QDateTime::fromString(inStr, Qt::ISODate) :QDateTime::fromString(inStr, SerializerSettings::getInstance()->getDateTimeFormat());
        if (dateTime.isValid()) {
...
bool fromJsonValue(QDateTime &value, const QJsonValue &jval) {
    ...
    if (!jval.isUndefined() && !jval.isNull() && jval.isString()) {
        value = SerializerSettings::getInstance()->getDateTimeFormat().isEmpty()?QDateTime::fromString(jval.toString(), Qt::ISODate): QDateTime::fromString(jval.toString(), SerializerSettings::getInstance()->getDateTimeFormat());
        ok = value.isValid();
    } else {
        ok = false;
    }
