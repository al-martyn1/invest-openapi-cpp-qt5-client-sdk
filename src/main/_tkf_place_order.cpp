...
INVEST_OPENAPI_MAIN()
{
    ...
    tkf_utils::OrderParams orderParams;

    int orderParamsParsingRes = tkf_utils::parseOrderParams( argsVec, dicts, orderParams );
    ...
    if (instrumentState.isValid() && instrumentGlass.isValid())
    {
        ...
        {
            unsigned lotSize   = unsigned(instrumentState.lotSize);
            unsigned orderSize = unsigned(orderParams.orderSize);

            unsigned numLots = orderSize / lotSize;
            ...
            {
                orderParams.orderType = tkf_utils::OrderParams::orderTypeMarket;
            }
            else if (orderParams.orderType==tkf_utils::OrderParams::orderTypeMarket)
            {
                // Do nothing
            }
            else // orderParams.orderType==orderTypeLimit or (Auto && SpreadPoints>1)
            {
                orderParams.orderType = tkf_utils::OrderParams::orderTypeLimit;
