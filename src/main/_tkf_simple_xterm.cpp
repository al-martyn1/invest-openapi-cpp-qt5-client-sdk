...
INVEST_OPENAPI_MAIN()
{
    ...
    while( !ctrlC.isBreaked() )
    {
        ...
        if (ordersResponse && ordersResponse->isFinished())
        {
            ...
            //NOTE: !!! Запрос активных заявок осуществляется скопом в один приём по всем инструментам
                     // Если заявок по инструменту нет, то в результат он не попадает, и получается, что я дальше 
                     // считаю, что просто нет обновлений. А на самом деле - нет заявок
                     // Поэтому заранее все известные инструменты прописываем в результат, а добавляется только
                     // по тем, по которым что-то есть

            terminalData.fillMapKeysWithFigis( activeOrders, std::vector<OpenAPI::Order>() );
    ...
    /*
       ...
       10) Почему-то LastBuyPr/LastSellPr не обновляются.
           Уже понял, почему - потому что операции по инструментам только один раз запрашиваются при старте, 
           а так как на каждую фигу нужен отдельный запрос - я собирался запрашивать их по мере 
           необходимости как-то, но так и не собрался.

           Чисто теоретически - можно запрашивать операции после того, как поубавилось активных заявок.
           Но - заявки запрашиваются с периодом - 10 секунд. За это время заявка может встать в очередь и 
           исполниться, и она просто не попадёт к нам. Также рыночная заявка не попадёт к нам в список активных.

           Еще, чисто теоретически, можно создать лимитную заявку и быстро её удалить - тоже не узнаем.

           а) Сделал, что при получении данных по заявкам можно узнать, изменился ли их состав

           б) Нужно на самом деле, сделать чекер возможности сделать запрос - 
              делаем deque, в которую при создании запроса кладём его милисек-стамп - в хвост.
              В основном цикле смотрим голову - если штамп старше минуты от текущего времени - извлекаем его.
              Затем смотрим - если размер меньше 120 - то можно делать новый запрос.

              Минута и 120 запросов - ограничения тинькова. Вообще, надо сделать какой-то класс (как назвать - QueryLimiter/RequestLimiter?),
              который можно параметризовать этими лимитами. А чтобы случайно не нарваться на лимит, снизим лимит по количеству - 
              110, например.

           в) Нужно не отсылать все запросы сразу, а ставить их в очередь. Извлекать, как только появляется возможность
              отослать. При этом - некоторые типы заменяют друг друга - например, запросы по инструменту или запрос активных ордеров,
              а некоторые могут дублироваться - размещение заявок на покупку/продажу.

           г) Нужно сделать очередь отложенных заявок - например, по получению ответа на размещение заявки на покупку/продажу через
              10 секунд надо запросить операции по инструменту.
              Вообще - это можно сделать в одной очереди, если штамп - ноль - исполняем, если больше штампа текущего времени - 
              пропускаем, и ищем дальше.

              Если заявка не успела к нам попасть - либо исполнилась, либо была быстро удалена - то отложенный запрос по инструменту
              исполнится. Если же список активных ордеров по инструменту обновился, то также будет сгенерирован запрос по инструменту,
              и он перетрёт уже лежащий в очереди отложенный запрос, в результате запросы не будут продублированы.

           д) Запросы должны обладать приоритетом - так, запросы на размещение заявок - имеют наивысший приоритет, и становятся
           в очередь сразу после аналогичных, но помещённых в очередь ранее.
       ...
       12) Нужно посчитать некий маркер/коэффициент/число активности для бумаги
           Количество поступивших по стриминг АПИ стаканов за какой-то промежуток времени

           - сделано - волатильность - количество обновлений стакана за последние 60 секунд.
