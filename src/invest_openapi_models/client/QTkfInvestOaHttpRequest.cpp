...
QTkfInvestOaHttpRequestInput::QTkfInvestOaHttpRequestInput() {
    initialize();
}

QTkfInvestOaHttpRequestInput::QTkfInvestOaHttpRequestInput(QString v_url_str, QString v_http_method) {
    initialize();
    url_str = v_url_str;
    http_method = v_http_method;
}

void QTkfInvestOaHttpRequestInput::initialize() {
    var_layout = NOT_SET;
    url_str = "";
    http_method = "GET";
}

void QTkfInvestOaHttpRequestInput::add_var(QString key, QString value) {
    vars[key] = value;
}

void QTkfInvestOaHttpRequestInput::add_file(QString variable_name, QString local_filename, QString request_filename, QString mime_type) {
    QTkfInvestOaHttpFileElement file;
...
QTkfInvestOaHttpRequestWorker::QTkfInvestOaHttpRequestWorker(QObject *parent)
    ...
    connect(manager, &QNetworkAccessManager::finished, this, &QTkfInvestOaHttpRequestWorker::on_manager_finished);
    timeOutTimer.setSingleShot(true);
}

QTkfInvestOaHttpRequestWorker::~QTkfInvestOaHttpRequestWorker() {
...
QMap<QString, QString> QTkfInvestOaHttpRequestWorker::getResponseHeaders() const {
    return headers;
}

QTkfInvestOaHttpFileElement QTkfInvestOaHttpRequestWorker::getHttpFileElement(const QString &fieldname) {
    if (!files.isEmpty()) {
    ...
    return QTkfInvestOaHttpFileElement();
}

QByteArray *QTkfInvestOaHttpRequestWorker::getMultiPartField(const QString &fieldname) {
...
void QTkfInvestOaHttpRequestWorker::setTimeOut(int timeOutMs) {
...
void QTkfInvestOaHttpRequestWorker::setWorkingDirectory(const QString &path) {
    if (!path.isEmpty()) {
        workingDirectory = path;
    }
}

void QTkfInvestOaHttpRequestWorker::setResponseCompressionEnabled(bool enable) {
    isResponseCompressionEnabled = enable;
}

void QTkfInvestOaHttpRequestWorker::setRequestCompressionEnabled(bool enable) {
    isRequestCompressionEnabled = enable;
}

int  QTkfInvestOaHttpRequestWorker::getHttpResponseCode() const{
    return httpResponseCode;
}

QString QTkfInvestOaHttpRequestWorker::http_attribute_encode(QString attribute_name, QString input) {
...
void QTkfInvestOaHttpRequestWorker::execute(QTkfInvestOaHttpRequestInput *input) {
    ...
    if (input->var_layout == ADDRESS || input->var_layout == URL_ENCODED) {
        ...
        // add files
        for (QList<QTkfInvestOaHttpFileElement>::iterator file_info = input->files.begin(); file_info != input->files.end(); file_info++) {
    ...
    if (input->request_body.size() > 0) {
        ...
        if(!isFormData && (input->var_layout != MULTIPART) && isRequestCompressionEnabled){
            request_content.append(compress(input->request_body, 7, QTkfInvestOaCompressionType::Gzip));
        } else {
    ...
    if (QTkfInvestOaHttpRequestWorker::sslDefaultConfiguration != nullptr) {
        request.setSslConfiguration(*QTkfInvestOaHttpRequestWorker::sslDefaultConfiguration);
    }
...
void QTkfInvestOaHttpRequestWorker::on_manager_finished(QNetworkReply *reply) {
    bool codeSts = false;
...
void QTkfInvestOaHttpRequestWorker::on_manager_timeout(QNetworkReply *reply) {
...
void QTkfInvestOaHttpRequestWorker::process_response(QNetworkReply *reply) {
    if (getResponseHeaders().contains(QString("Content-Disposition"))) {
        ...
        if ((contentDisposition.count() > 0) && (contentDisposition.first() == QString("attachment"))) {
            ...
            QTkfInvestOaHttpFileElement felement;
...
QByteArray QTkfInvestOaHttpRequestWorker::decompress(const QByteArray& data){
    
    Q_UNUSED(data);
    return QByteArray();
}

QByteArray QTkfInvestOaHttpRequestWorker::compress(const QByteArray& input, int level, QTkfInvestOaCompressionType compressType) {
...
QSslConfiguration *QTkfInvestOaHttpRequestWorker::sslDefaultConfiguration;
